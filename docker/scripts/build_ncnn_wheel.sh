...
python3.11 -m pip install --break-system-packages --upgrade pip setuptools wheel
mkdir -p /src /wheels
...
if [[ -f setup.py ]]; then
  python3.11 setup.py bdist_wheel --dist-dir /wheels
else
  python3.11 -m pip wheel --break-system-packages . --wheel-dir /wheels
fi
